...
function scene:create( event )
 ...
 --    title:addEventListener("tap", onSecondView ) 
	...
	local teks_kecantikan = display.newText( "KECANTIKAN", display.contentCenterX - display.contentWidth / 5, display.contentHeight/25 , used_font_bold, 12  * (display.contentWidth / 320))
	teks_kecantikan.anchorX = 0.5
	teks_kecantikan:setFillColor(133/255,190/255,72/255)
	scrollView:insert(teks_kecantikan)

	local function onTeks_kecantikan(event) 
	            composer.status_kategori = 4
	            but_teks_kecantikan(teks_kecantikan)
	          return true 
	       end 
	       teks_kecantikan:addEventListener("tap", onTeks_kecantikan ) 

   function but_teks_kecantikan(self) 
      
            if nil~= composer.getScene("menu3") then composer.removeScene("menu3", true) end  
            if  self.type == "press" then 
                self:setEnabled(false) 
            else 
                self:removeEventListener( "tap", onTeks_kecantikan ) 
            end 
            composer.gotoScene( "menu3" ) 
        
   end 

	local teks_makanan = display.newText( "MAKANAN", display.contentCenterX + display.contentWidth / 5, display.contentHeight/25 , used_font_bold, 12  * (display.contentWidth / 320))
	teks_makanan.anchorX = 0.5
	teks_makanan:setFillColor(133/255,190/255,72/255)
	scrollView:insert(teks_makanan)
	

	local function onTeks_makanan(event) 
	            composer.status_kategori = 5
	            but_teks_makanan(teks_makanan)
	          return true 
	       end 
	       teks_makanan:addEventListener("tap", onTeks_makanan ) 

   function but_teks_makanan(self) 
      
            if nil~= composer.getScene("menu3") then composer.removeScene("menu3", true) end  
            if  self.type == "press" then 
                self:setEnabled(false) 
            else 
                self:removeEventListener( "tap", onTeks_makanan ) 
            end 
            composer.gotoScene( "menu3" ) 
        
   end 

   local teks_hemat = display.newText( "PAKET HEMAT", display.contentCenterX - display.contentWidth / 3, teks_makanan.y + teks_makanan.height + 10, used_font_bold, 12  * (display.contentWidth / 320))
	teks_hemat.anchorX = 0.5
	teks_hemat:setFillColor(133/255,190/255,72/255)
	scrollView:insert(teks_hemat)

	local function onTeks_hemat(event) 
	            composer.status_kategori = 6
	            but_teks_hemat(teks_hemat)
	          return true 
	       end 
	       teks_hemat:addEventListener("tap", onTeks_hemat ) 

   function but_teks_hemat(self) 
      
            if nil~= composer.getScene("menu3") then composer.removeScene("menu3", true) end  
            if  self.type == "press" then 
                self:setEnabled(false) 
            else 
                self:removeEventListener( "tap", onTeks_hemat ) 
            end 
            composer.gotoScene( "menu3" ) 
   ...
   local teks_souvenir = display.newText( "SOUVENIR", display.contentCenterX, teks_makanan.y + teks_makanan.height + 10, used_font_bold, 12  * (display.contentWidth / 320))
	teks_souvenir.anchorX = 0.5
	teks_souvenir:setFillColor(133/255,190/255,72/255)
	scrollView:insert(teks_souvenir)

	local function onTeks_souvenir(event) 
	            composer.status_kategori = 7
	            but_teks_souvenir(teks_souvenir)
	          return true 
	       end 
	       teks_souvenir:addEventListener("tap", onTeks_souvenir ) 

   function but_teks_souvenir(self) 
      
            if nil~= composer.getScene("menu3") then composer.removeScene("menu3", true) end  
            if  self.type == "press" then 
                self:setEnabled(false) 
            else 
                self:removeEventListener( "tap", onTeks_souvenir ) 
            end 
            composer.gotoScene( "menu3" ) 
   ...
   local teks_parfum = display.newText( "PARFUM", display.contentCenterX + display.contentWidth / 3, teks_makanan.y + teks_makanan.height + 10, used_font_bold, 12  * (display.contentWidth / 320))
	teks_parfum.anchorX = 0.5
	teks_parfum:setFillColor(133/255,190/255,72/255)
	scrollView:insert(teks_parfum)

	local function onTeks_parfum(event) 
	            composer.status_kategori = 8
	            but_teks_parfum(teks_parfum)
	          return true 
	       end 
	       teks_parfum:addEventListener("tap", onTeks_parfum ) 

   function but_teks_parfum(self) 
      
            if nil~= composer.getScene("menu3") then composer.removeScene("menu3", true) end  
            if  self.type == "press" then 
                self:setEnabled(false) 
            else 
                self:removeEventListener( "tap", onTeks_parfum ) 
            end 
            composer.gotoScene( "menu3" ) 
	...
	local terbaru_box = display.newRect( display.contentCenterX, teks_parfum.y + display.contentHeight/10, display.contentWidth * 0.8, 50 )
...
function scene:show( event )
          ...
          local index
             ...
             for index = 1, data_max do
                  ...
                  local data = listData[index]
                    ...
                    local count = composer.gambar_iklan_olehhaji
                    ...
                    local myImage = display.newImage( composer.imgDir.. "logo.png", 125, 39)
                      ...
                      local posisi_teks_x = myImage.x + (myImage.width * myImage.xScale /2)
                       ...
                       if(composer.gambar_iklan_olehhaji%2 == 1)then
                      ...
                      local teks  = table.concat(composer.data_olehhaji, ", ", composer.gambar_iklan_olehhaji,composer.gambar_iklan_olehhaji)
                      local teks2 = table.concat(composer.data_olehhaji_harga, ", ", composer.gambar_iklan_olehhaji,composer.gambar_iklan_olehhaji)
					...
					myImage:addEventListener("tap", onProduk )
			         ...
			         scrollView:insert(teks_produk_diskon_harga);
                      ...
                      if(composer.gambar_iklan_olehhaji == data_max)then
	...
	local function ontampil_next(event) 
	   ...
	   function but_tampil_prev(self) 
	            ...
	            composer.halaman_page = composer.halaman_page - 10
                    ...
                    cancelall[index] = network.download( download, "GET", networkListener,params, "olehhaji"..index..".jpg", system.TemporaryDirectory, 0, 0 )
	...
	if(composer.status_kategori == 1)then
			dl_id1 = network.request( "http://api.bursasajadah.com/api/bursasajadah/filter/139/-/terbaru/-/10/"..composer.halaman_page, "GET", dataResponseListenerOlehHaji )    		
	elseif(composer.status_kategori == 2)then
			dl_id1 = network.request( "http://api.bursasajadah.com/api/bursasajadah/filter/139/terendah/terbaru/-/10/"..composer.halaman_page, "GET", dataResponseListenerOlehHaji ) 
	elseif(composer.status_kategori == 3)then
    		dl_id1 = network.request( "http://api.bursasajadah.com/api/bursasajadah/filter/139/-/-/terlaris/10/"..composer.halaman_page, "GET", dataResponseListenerOlehHaji) 
	elseif(composer.status_kategori == 4)then
			dl_id1 = network.request( "http://api.bursasajadah.com/api/bursasajadah/produkcategory/kecantikan/10/"..composer.halaman_page, "GET", dataResponseListenerOlehHaji)	
	elseif(composer.status_kategori == 5)then
			dl_id1 = network.request( "http://api.bursasajadah.com/api/bursasajadah/produkcategory/makanan/10/"..composer.halaman_page, "GET", dataResponseListenerOlehHaji)
	elseif(composer.status_kategori == 6)then
			dl_id1 = network.request( "http://api.bursasajadah.com/api/bursasajadah/produkcategory/paket-hemat/10/"..composer.halaman_page, "GET", dataResponseListenerOlehHaji)
	elseif(composer.status_kategori == 7)then
			dl_id1 = network.request( "http://api.bursasajadah.com/api/bursasajadah/produkcategory/souvenir/10/"..composer.halaman_page, "GET", dataResponseListenerOlehHaji)
	elseif(composer.status_kategori == 8)then
			dl_id1 = network.request( "http://api.bursasajadah.com/api/bursasajadah/produkcategory/parfum/10/"..composer.halaman_page, "GET", dataResponseListenerOlehHaji)
	end
