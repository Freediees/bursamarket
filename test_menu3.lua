...
function scene:create( event )
 ...
 --    title:addEventListener("tap", onSecondView ) 
  ...
  composer.tinggi_teksfield = background2.height + display.contentHeight/30
	...
	local teks_tipe = display.newText( "KECANTIKAN", display.contentCenterX - 10, teks_sajadah.y + display.contentHeight/20 , used_font, 12  * (display.contentWidth / 320))
	teks_tipe.anchorX = 1
	teks_tipe:setFillColor(1)

	local back_tipe = display.newRoundedRect(teks_tipe.x - teks_tipe.width/2, teks_tipe.y, teks_tipe.width + 10, teks_tipe.height + 10, 5)
	back_tipe.anchorX = 0.5
   ...
   function but_teks_tipe(self) 
   	...
   	local teks_ukuran = display.newText( "MAKANAN", display.contentCenterX + 10, teks_sajadah.y + display.contentHeight/20 , used_font, 12  * (display.contentWidth / 320))
	teks_ukuran.anchorX = 0
	teks_ukuran:setFillColor(1)

	local back_ukuran = display.newRoundedRect(teks_ukuran.x + teks_ukuran.width/2, teks_ukuran.y, teks_ukuran.width + 10, teks_ukuran.height + 10, 5)
	back_ukuran.anchorX = 0.5
   ...
   local teks_hemat = display.newText( "PAKET HEMAT", display.contentCenterX, back_ukuran.y + back_ukuran.height + 10, used_font, 12  * (display.contentWidth / 320))
	teks_hemat.anchorX = 0.5
	teks_hemat:setFillColor(1)

	local back_hemat = display.newRoundedRect(teks_hemat.x, teks_hemat.y, teks_hemat.width + 10, teks_hemat.height + 10, 5)
	back_hemat.anchorX = 0.5
	back_hemat:setFillColor(133/255,190/255,72/255)

	local function onTeks_hemat(event) 
		--composer.teks_sorting = 
        composer.status_kategori = 6
        but_teks_ukuran(teks_hemat)
      return true 
   end 
   teks_hemat:addEventListener("tap", onTeks_hemat ) 

	
	scrollView:insert(back_hemat)
	scrollView:insert(teks_hemat)

	local teks_souvenir = display.newText( "SOUVENIR", back_hemat.x - back_hemat.width/2 - 10, back_hemat.y, used_font, 12  * (display.contentWidth / 320))
	teks_souvenir.anchorX = 1
	teks_souvenir:setFillColor(1)

	local back_souvenir = display.newRoundedRect(teks_souvenir.x - teks_souvenir.width/2, teks_souvenir.y, teks_souvenir.width + 10, teks_souvenir.height + 10, 5)
	back_souvenir.anchorX = 0.5
	back_souvenir:setFillColor(133/255,190/255,72/255)
	
	scrollView:insert(back_souvenir)
   ...
   teks_souvenir:addEventListener("tap", onTeks_souvenir ) 
	...
	local teks_parfum = display.newText( "PARFUM", back_hemat.x + back_hemat.width/2 + 10, back_hemat.y, used_font, 12  * (display.contentWidth / 320))
	teks_parfum.anchorX = 0
	teks_parfum:setFillColor(1)

	local back_parfum = display.newRoundedRect(teks_parfum.x + teks_parfum.width/2, teks_parfum.y, teks_parfum.width + 10, teks_parfum.height + 10, 5)
	back_parfum.anchorX = 0.5
	back_parfum:setFillColor(133/255,190/255,72/255)

	local function onTeks_parfum(event) 
        composer.status_kategori = 8
        but_teks_ukuran(teks_parfum)
      return true 
   end 
   teks_parfum:addEventListener("tap", onTeks_parfum ) 

	
	scrollView:insert(back_parfum)
	scrollView:insert(teks_parfum)



   local teks_sorting = display.newText( "Sorting", 20,  teks_hemat.y + display.contentWidth/10, used_font_bold, 12  * (display.contentWidth / 320))
