...
function scene:create( event )
	...
	background2:setFillColor( 168/255,123/255,80/255)
	...
	local teks_daftar_isi = display.newText( "Lain-Lain", menu.x + display.contentWidth/8 , menu.y , used_font_bold, 20  * (display.contentWidth / 320)  )
 ...
 --    teks_umroh:addEventListener("tap", onteks_umroh) 
	...
	for row in db:nrows("SELECT * FROM t_ceklis4") do
		...
		function on_option1(self) 
			...
			if(status_cek == 1)then
				...
				local tablesetup = [[UPDATE t_ceklis4 SET status = '0' where value = ']]..row.value..[[';]]
				db:exec(tablesetup)
				...
				local tablesetup = [[UPDATE t_ceklis4 SET status = '1' where value = ']]..row.value..[[';]]
				db:exec(tablesetup)
	...
	local function tambah_perlengkapan()
		...
		composer.showOverlay("overlay_popup4", options_overlay)
	end
