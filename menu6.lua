...
function scene:create( event )
 ...
 --    title:addEventListener("tap", onSecondView ) 
	...
	local teks_anak = display.newText( "BUSANA ANAK", display.contentCenterX  - display.contentWidth/5, display.contentHeight/20 , used_font_bold, 12  * (display.contentWidth / 320))
	teks_anak.anchorX = 0.5
	teks_anak:setFillColor(133/255,190/255,72/255)
	scrollView:insert(teks_anak)

	local function onTeks_anak(event) 
	            composer.status_kategori = 4
	            but_teks_anak(teks_anak)
	          return true 
	       end 
	       teks_anak:addEventListener("tap", onTeks_anak ) 

   function but_teks_anak(self) 
      
            if nil~= composer.getScene("menu6") then composer.removeScene("menu6", true) end  
            if  self.type == "press" then 
                self:setEnabled(false) 
            else 
                self:removeEventListener( "tap", onTeks_anak ) 
            end 
            composer.gotoScene( "menu6" ) 
        
   end 

	local teks_pria = display.newText( "BUSANA PRIA", display.contentCenterX  + display.contentWidth/5, display.contentHeight/20 , used_font_bold, 12  * (display.contentWidth / 320))
	teks_pria.anchorX = 0.5
	teks_pria:setFillColor(133/255,190/255,72/255)
	scrollView:insert(teks_pria)
	

	local function onTeks_pria(event) 
	            composer.status_kategori = 5
	            but_teks_pria(teks_pria)
	          return true 
	       end 
	       teks_pria:addEventListener("tap", onTeks_pria ) 

   function but_teks_pria(self) 
      
            if nil~= composer.getScene("menu6") then composer.removeScene("menu6", true) end  
            if  self.type == "press" then 
                self:setEnabled(false) 
            else 
                self:removeEventListener( "tap", onTeks_pria ) 
            end 
            composer.gotoScene( "menu6" ) 
   ...
   local teks_wanita = display.newText( "BUSANA WANITA", display.contentCenterX  - display.contentWidth/5, teks_anak.y + display.contentHeight/20 , used_font_bold, 12  * (display.contentWidth / 320))
	teks_wanita.anchorX = 0.5
	teks_wanita:setFillColor(133/255,190/255,72/255)
	scrollView:insert(teks_wanita)

	local function onTeks_wanita(event) 
	            composer.status_kategori = 6
	            but_teks_wanita(teks_wanita)
	          return true 
	       end 
	       teks_wanita:addEventListener("tap", onTeks_wanita ) 

   function but_teks_wanita(self) 
      
            if nil~= composer.getScene("menu6") then composer.removeScene("menu6", true) end  
            if  self.type == "press" then 
                self:setEnabled(false) 
            else 
                self:removeEventListener( "tap", onTeks_wanita ) 
            end 
            composer.gotoScene( "menu6" ) 
        
   end 

	local teks_sykava = display.newText( "SYKAVA", display.contentCenterX  + display.contentWidth/5, teks_anak.y + display.contentHeight/20 , used_font_bold, 12  * (display.contentWidth / 320))
	teks_sykava.anchorX = 0.5
	teks_sykava:setFillColor(133/255,190/255,72/255)
	scrollView:insert(teks_sykava)
	

	local function onTeks_sykava(event) 
	            composer.status_kategori = 7
	            but_teks_sykava(teks_sykava)
	          return true 
	       end 
	       teks_sykava:addEventListener("tap", onTeks_sykava ) 

   function but_teks_sykava(self) 
      
            if nil~= composer.getScene("menu6") then composer.removeScene("menu6", true) end  
            if  self.type == "press" then 
                self:setEnabled(false) 
            else 
                self:removeEventListener( "tap", onTeks_sykava ) 
            end 
            composer.gotoScene( "menu6" ) 
   ...
   local teks_zaleera = display.newText( "ZALEERA", display.contentCenterX  - display.contentWidth/5, teks_wanita.y + display.contentHeight/20 , used_font_bold, 12  * (display.contentWidth / 320))
	teks_zaleera.anchorX = 0.5
	teks_zaleera:setFillColor(133/255,190/255,72/255)
	scrollView:insert(teks_zaleera)

	local function onTeks_zaleera(event) 
	            composer.status_kategori = 8
	            but_teks_zaleera(teks_zaleera)
	          return true 
	       end 
	       teks_zaleera:addEventListener("tap", onTeks_zaleera ) 

   function but_teks_zaleera(self) 
      
            if nil~= composer.getScene("menu6") then composer.removeScene("menu6", true) end  
            if  self.type == "press" then 
                self:setEnabled(false) 
            else 
                self:removeEventListener( "tap", onTeks_zaleera ) 
            end 
            composer.gotoScene( "menu6" ) 
        
   end 

	local teks_syaheera = display.newText( "SYAHEERA", display.contentCenterX  + display.contentWidth/5, teks_wanita.y + display.contentHeight/20 , used_font_bold, 12  * (display.contentWidth / 320))
	teks_syaheera.anchorX = 0.5
	teks_syaheera:setFillColor(133/255,190/255,72/255)
	scrollView:insert(teks_syaheera)
	

	local function onTeks_syaheera(event) 
	            composer.status_kategori = 9
	            but_teks_syaheera(teks_syaheera)
	          return true 
	       end 
	       teks_syaheera:addEventListener("tap", onTeks_syaheera ) 

   function but_teks_syaheera(self) 
      
            if nil~= composer.getScene("menu6") then composer.removeScene("menu6", true) end  
            if  self.type == "press" then 
                self:setEnabled(false) 
            else 
                self:removeEventListener( "tap", onTeks_syaheera ) 
            end 
            composer.gotoScene( "menu6" ) 
	...
	local terbaru_box = display.newRect( display.contentCenterX, teks_syaheera.y + display.contentHeight/10, display.contentWidth * 0.8, 50 )
	if(composer.status_kategori == 1 or composer.status_kategori == 4 or composer.status_kategori == 5)then
             ...
             for index = 1, data_max do
                  ...
                  local data = listData[index]
                    ...
                    local myImage = display.newImage( composer.imgDir.. "logo.png", 125, 39)  
                      ...
                      local teks  = table.concat(composer.data_busanamuslim, ", ", composer.gambar_iklan_busanamuslim,composer.gambar_iklan_busanamuslim)
                      local teks2 = table.concat(composer.data_busanamuslim_harga, ", ", composer.gambar_iklan_busanamuslim,composer.gambar_iklan_busanamuslim)
                   ...
                   function onProduk( event )
					...
					myImage:addEventListener("tap", onProduk )
			         ...
			         scrollView:insert(teks_produk_diskon_harga);
                      ...
                      if(composer.gambar_iklan_busanamuslim == data_max)then
	...
	local function ontampil_next(event) 
	   ...
	   function but_tampil_prev(self) 
	            ...
	            composer.halaman_page = composer.halaman_page - 10
                   ...
                   cancelall[index] = network.download( download, "GET", networkListener,params, "busanamuslim"..index..".jpg", system.TemporaryDirectory, 0, 0 )
