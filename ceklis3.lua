...
function scene:create( event )
	...
	background2:setFillColor( 102/255,45/255,145/255)
	...
	local teks_daftar_isi = display.newText( "Obat & Vitamin", menu.x + display.contentWidth/8 , menu.y , used_font_bold, 20  * (display.contentWidth / 320)  )
 ...
 --    teks_umroh:addEventListener("tap", onteks_umroh) 
	...
	for row in db:nrows("SELECT * FROM t_ceklis3") do
		...
		function on_option1(self) 
			...
			if(status_cek == 1)then
				...
				local tablesetup = [[UPDATE t_ceklis3 SET status = '0' where value = ']]..row.value..[[';]]
				db:exec(tablesetup)
				...
				local tablesetup = [[UPDATE t_ceklis3 SET status = '1' where value = ']]..row.value..[[';]]
				db:exec(tablesetup)
	...
	local function tambah_perlengkapan()
		...
		composer.showOverlay("overlay_popup3", options_overlay)
	end
