...
function scene:show( event )
	...
	elseif phase == "did" then
		composer.removeHidden()
     	...
     	local back1 = display.newRoundedRect(display.contentCenterX, display.contentCenterY + display.contentHeight/8 , 0.8 * display.contentWidth, 30, 5 )
     	back1:setFillColor(0.9)
     	sceneGroup:insert(back1)

     	teksfield_email = native.newTextField( display.contentCenterX, display.contentCenterY + display.contentHeight/8 , 0.75 * display.contentWidth, 40 )
		teksfield_email.anchorX = 0.5
		teksfield_email.anchorY = 0.5
		teksfield_email.alpha = 1
		teksfield_email.size = 12  * (display.contentWidth / 320)
		teksfield_email.placeholder = "Email"
		teksfield_email.inputType = "email"
		teksfield_email.hasBackground = false
		teksfield_email:resizeHeightToFitFont()
		sceneGroup:insert( teksfield_email )

		local function fieldHandler_t(event) 
	               if ("began" == event.phase) then

	               	  sceneGroup.y = sceneGroup.y - (0.25 * display.contentHeight)
	               ...
	               elseif ("submitted" == event.phase) then  
	                  ...
	                  --sceneGroup.y = sceneGroup.y + (0.3 * display.contentHeight)
	    ...
	    local back2 = display.newRoundedRect(display.contentCenterX, teksfield_email.y + teksfield_email.height + 20 , 0.8 * display.contentWidth, 30, 5 )
     	back2:setFillColor(0.9)
     	sceneGroup:insert(back2)

		teksfield_password = native.newTextField( display.contentCenterX, teksfield_email.y + teksfield_email.height + 20 , 0.75 * display.contentWidth, 40 )
		teksfield_password.anchorX = 0.5
		teksfield_password.anchorY = 0.5
		teksfield_password.alpha = 1
		teksfield_password.size = 12  * (display.contentWidth / 320)
		teksfield_password.placeholder = "Password"
		teksfield_password.isSecure = true
		teksfield_password.hasBackground = false
		teksfield_password:resizeHeightToFitFont()
	...
	local function onLoginView()
		...
		composer.showOverlay("overlay_loading", options_overlay)
		    ...
		    local background_login = display.newRoundedRect( display.contentCenterX, teksfield_password.y + display.contentHeight/12 , 0.8 * display.contentWidth, 40, 5  )
			...
			local function daftar()
				...
				local popupOptions =
				{
				    url = "http://m.bursasajadah.com/register",
				}
...
function scene:hide( event )
	...
	if event.phase == "will" then
		...
		if(teksfield_password ~=nil)then
		teksfield_password:removeSelf()
		teksfield_password = nil
		end
